...
function Home() {
    ...
    const stages = [
        { title: t("home.stage.1.title"), text: t("home.stage.1.text") },
        { title: t("home.stage.2.title"), text: t("home.stage.2.text") },
        { title: t("home.stage.3.title"), text: t("home.stage.3.text") },
    ];
                ...
                <div className="min-w-0 max-w-3xl animate-soft-rise">
                    ...
                    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                        {isAuthenticated ? (
                            isOrganizer ? (
                                <>
                                    <Button to="/dashboard" size="lg">{t("home.toDashboard")}</Button>
                                    <Button to="/dashboard/organizer/create" variant="secondary" size="lg">{t("home.createQuiz")}</Button>
                                </>
                            ) : (
                                <>
                                    <Button to="/dashboard" size="lg">{t("home.toDashboard")}</Button>
                                    <Button to="/dashboard/participant/history" variant="secondary" size="lg">{t("home.myHistory")}</Button>
                                </>
                            )
                        ) : (
                            <>
                                <Button to="/register" size="lg">{t("home.startGame")}</Button>
                                <Button to="/login" variant="secondary" size="lg">{t("home.login")}</Button>
                            </>
                        )}
                    </div>
                </div>
                ...
                <div className="min-w-0 animate-soft-rise rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl shadow-slate-200/70 [animation-delay:120ms] sm:rounded-[2rem] sm:p-4">
                    <div className="rounded-3xl border border-brand-100 bg-brand-50 p-3 sm:rounded-[1.5rem] sm:p-4">
                        <div className="flex items-start justify-between gap-3 sm:items-center sm:gap-4">
                            <div>
                                <p className="text-xs font-black uppercase tracking-[0.14em] text-brand-700 sm:text-sm sm:tracking-[0.16em]">{t("home.weeklyLeaderboard")}</p>
                                <h2 className="mt-1 text-lg font-black text-slate-950 sm:text-xl">{t("home.activeParticipants")}</h2>
                            </div>
                            <div className="hidden rounded-full bg-white px-3 py-1 text-xs font-black text-brand-700 shadow-sm sm:block sm:text-sm">
                                Live
                            </div>
                        </div>
                                    ...
                                    <div className="flex items-center gap-3 sm:gap-4">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center justify-between gap-3">
                                                <h3 className="truncate font-black text-slate-950">{t(entry.nameKey)}</h3>
                                                <span className="hidden shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700 sm:inline">
                                                    {t("home.quizzesCount", { count: entry.quizzes })}
                                                </span>
                                            </div>
                                        ...
                                        <div className="hidden shrink-0 text-right sm:block">
                                            ...
                                            <p className="text-xs font-semibold text-slate-500">{t("home.points")}</p>
                                        </div>
                                    </div>
                                </article>
                            ))}
